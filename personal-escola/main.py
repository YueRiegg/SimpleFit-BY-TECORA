import sys
# sys serve para interagir com o python e com o sistema que o programa tá rodando.
...
#        ^ importando tudo que é importante.
def menu_inicial():
    while True:
        print("\n===== PERSONAL VIRTUAL =====")
        print("1 - Login")
...
#                                                ^ tira os espaços e quebra de linha no início e no fim do texto
        if escolha == "1":
            login = Login()
...
#                            ↓ verifica se começa com s, e como o input foi definido como .lower(), se o usuário
# escrever SIMM ele também   ↓   daria certo
...
#      ^ defini todas as variáveis como string para evitar que cause erro caso a pessoa escreva "70kg" por exemplo
...
def loop_chat(perfil):
    ...
    while True:
        ...
        if mensagem.lower() == "sair":
            print("\nAté a próxima!")
            break
#            ^ verifica se o usuário disse "sair", se sim ele encerra o chatbot, e se não ele continua
        if not mensagem:
            continue
...
def main():
    ...
    print(dieta(perfil))
    loop_chat(perfil)
#        ↓ verifica se o nome do arquivo é "main.py" e se for ele funciona, mas se for herdado por outro arquivo, ele não funciona.
